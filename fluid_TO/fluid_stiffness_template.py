...
class FluidStiffnessTemplates:
  
  def __init__(self, elem_dx:float, elem_dy:float, fluid_mat_constants:material_constants.MaterialConstants):
    (self.A_mu, self.B, self.mat_area, self.Aalpha_X, 
     self.Aalpha_Y, self.Aalpha_XY) = computeFluidMatrices(elem_dx,elem_dy,fluid_mat_constants.kinematic_viscosity)

def computeFluidMatrices(dx:float, dy:float, mu0:float) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:

  """
  Computes various matrices related to fluid dynamics given the grid spacing
  (dx, dy) and the dynamic viscosity mu0.
  https://link.springer.com/article/10.1007/s00158-014-1182-z
  For description of fluid matrices.
  Args:
  - dx: Grid spacing in the x-direction.
  - dy: Grid spacing in the y-direction.
  - mu0: Dynamic viscosity.

  Returns:
  Tuple: A tuple containing the following matrices:
      - Amu: Matrix related to viscous stress.
      - B: Matrix B is the gradient coefficient matrix
      - matArea: Matrix representing the area of element.
      - AalphaX: Matrix AalphaX is inverse permeability coefficient in x-direction.
      - AalphaY: Matrix AalphaY is inverse permeability coefficient in y-direction.
      - AalphaXY: Matrix AalphaXY is inverse permeability coefficient in xy-direction.
  """
  t2 = dx**2;
  t3 = dy**2;
  t4 = 1.0/dx;
  ...
  return (utils.to_torch(Amu), utils.to_torch(B), utils.to_torch(matArea),
          utils.to_torch(AalphaX), utils.to_torch(AalphaY), utils.to_torch(AalphaXY))