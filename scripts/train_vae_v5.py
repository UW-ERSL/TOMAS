"""Train VAE v5: dataset_num=3 (m in [2,8]) + relaxed filter.

v5 differences from v4:
  - Uses dataset_num=3 (m restricted to [2,8] -> mostly circular/regular shapes,
    avoiding the star-shaped artifacts that dominated v2/v3/v4 with m up to 22).
  - Same filter as v4: area >= 0.03, perim >= 0.5.

Outputs: vae/vae_net_v5.pt, vae/normalization_v5.pt, vae/training_history_v5.png
"""
import sys
import time
...
def main():
  ...
  norm_path = ROOT / "vae" / "normalization_v5.pt"
  ...
  net_path = ROOT / "vae" / "vae_net_v5.pt"
  ...
  ax.set_title(f"VAE v5 training (m∈[2,8], {num_samples} filtered samples)")
  fig.tight_layout()
  fig.savefig(ROOT / "vae" / "training_history_v5.png", dpi=180)
  plt.close(fig)
